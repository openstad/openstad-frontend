...
// TODO: more text
// ----------------------------------------------------------------------------------------------------
// Todo: Map -> refactor to one interface for every map type
function GoogleMaps( markerStyle, polygonStyle, editorInputElement, editorMarker ) {
	...
	if (editorInputElement) {
		...
		self.getLocation = function() {
			var locationToParse = self.editorInputElement && self.editorInputElement.value ? self.editorInputElement.value : null;

			var point = JSON.parse(locationToParse);
		...
		self.editorInputElement = document.getElementById(editorInputElement);
...
GoogleMaps.prototype.createMap = function( config, markers, polygon, autoZoomAndCenter ) {
  ...
  self.config = config;
	...
	self.initGoogleMaps = function () {
			...
			if (mapEl) {
				// init map
				self.map = new google.maps.Map(mapEl, config);
	...
	// if google isset init, otherwise load the google maps script first
...
GoogleMaps.prototype.createCutoutPolygon = function( polygon ) {
...
GoogleMaps.prototype.createMarkers = function( markers ) {
...
GoogleMaps.prototype.createMarker = function( marker ) {
	...
	if (marker.href && this.config.useMarkerLinks) {
		marker.icon.clickable = this.config.useMarkerLinks ? true : false;
	}
    ...
    cursor      : this.config.useMarkerLinks ? 'pointer' : 'drag',
	}
...
GoogleMaps.prototype.setBoundsAndCenter = function( points ) {
