  ...
  construct: function (self, options) {
    options.arrangeFields = (options.arrangeFields || []).concat([
      ...
      },
      {
        name: 'submitting',
        label: 'Submitting',
        fields: ['buttonTextSubmit', 'buttonTextConcept'],
      },
      {
        name: 'agreed',
    ...
    self.load = function (req, widgets, next) {
      ...
      widgets.forEach(async (widget) => {
        ...
        const resourceConfig =
          siteConfig && siteConfig[resourceConfigKey]
            ? siteConfig[resourceConfigKey]
            : {};
        ...
        if (activeResource && activeResource.extraData.budgetDocuments) {
          try {
            activeResource.extraData.budgetDocuments = JSON.parse(
              activeResource.extraData.budgetDocuments
            );
          } catch (error) {}
        }
        ...
        const isOwner = activeResource
          ? req.data.openstadUser &&
            req.data.openstadUser.id === activeResource.userId
          : false;
        const isReactedTo = activeResource
          ? activeResource.yes > 0 ||
            activeResource.no > 0 ||
            activeResource.argumentCount > 0
          : false;
        const isOwnerOrAdmin =
          ((!isReactedTo || !widget.hideAdminAfterPublicAction) && isOwner) ||
          req.data.hasModeratorRights;

        widget.isTagSelected = function (tagId) {
          if (!activeResource || !Array.isArray(activeResource.tags))
            return false;
          ...
          for (const type of ['User', 'Admin']) {
            if (widget[`confirmationEnabled${type}`]) {
              ...
              const { template, recipient } =
                await self.getNotificationByFormName(formName);

              widget[`confirmationTemplateName${type}`] =
                template.templateFile || '';
              widget[`confirmationSubject${type}`] = template.subject || '';
              widget[`confirmationEmailField${type}`] =
                self.removeColumnNameFromEmailFieldValue(recipient.value);
    ...
    self.route('post', 'modal', function (req, res) {
      ...
      return res.send(
        self.render(req, 'widgetEditor.html', {
          label: self.label,
          schema: schema,
        })
      );
