...
function OpenStadMapForm( markerStyle, polygonStyle, editorInputElement, editorMarker ) {
	...
	if (editorInputElement) {
		...
		self.getLocation = function() {
			...
			if( point ) {
				return {lat: parseFloat(point.coordinates[0]), lng: parseFloat(point.coordinates[1])}
			} else {
				return null;
			}
		};
...
OpenStadMapForm.prototype.createMap = function( config, markers, polygon, autoZoomAndCenter ) {
	...
	self.initGoogleMaps = function () {
		...
		if (config.center) {
			config.center = new google.maps.LatLng(config.center.lat,config.center.lng);
		}
...
OpenStadMapForm.prototype.createCutoutPolygon = function( polygon ) {
...
OpenStadMapForm.prototype.createMarkers = function( markers ) {
...
OpenStadMapForm.prototype.createMarker = function( marker ) {
...
OpenStadMapForm.prototype.setBoundsAndCenter = function( points ) {
