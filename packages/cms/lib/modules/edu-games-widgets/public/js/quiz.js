    ...
    base.methods = {
      ...
      setup: function() {
        ...
        $.each(questions, function(i, question) {
          ...
          quizHtml +=  question.sound ? '<audio controls class="question-audio"><source src="'+ question.sound +'" type="audio/mpeg"></audio>' : '';
          quizHtml += question.image ? '<div class="question-image-container"> <img src="'+question.image+'" /></div>' : '';
          quizHtml += question.displayTitle ? '<p class="question question-title">' + question.q + '</p>' : '';
          quizHtml += '<ul class="answers">';
          $.each(question.options, function(index, answer) {
            var answerImage = answer.image && answer.image.length > 0 ? '<span class="answer-image"> <img src="'+answer.image+'" /></span>' : '';
            var isCorrect = answer.isCorrect === true ? 'true' : 'false';
            quizHtml += '<li><a href="#" data-index="' + index + '" data-is-correct="'+isCorrect +'">' + answerImage+ answer.label + '</a></li>';
          });
          quizHtml += '</ul>';
          quizHtml += '</div>';
        });
      ...
      start: function() {
        ...
        var $audioEls = $('.active-question').find('audio');
      ...
      answerQuestion: function(answerEl) {
        ...
        var $answerEl = $(answerEl),
          ...
          isCorrect =  $answerEl.data('is-correct');

          console.log('isCorrect', isCorrect);
        // if set to true as a string
        if (isCorrect === true) {
      ...
      updateCounter: function() {
        console.log('base.options.counterFormat', base.options.counterFormat);
