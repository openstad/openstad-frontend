...
apos.define('participatory-budgeting-widgets', {
  ...
  function displayIdeaOnHash ($widget) {
    ...
    // }
	  ...
	  var isOpen =  $widget.find('#idea-' + ideaId).hasClass('selectedItem');
	  var scrollToTop;
	  var stickyHeight = $(window).width() > 767 ? 76 : 109;

	  if (isOpen && $widget.find('.gridder-show').length > 0) {

		  scrollToTop = $widget.find('.gridder-show').offset().top - stickyHeight - 12;
        ...
        scrollTop: scrollToTop
		  ...
		  if (ideaId && document.querySelector('#idea-' + ideaId) && document.querySelector('#idea-' + ideaId).querySelector('.button-read-more')) {
			  ...
			  setTimeout(function() {
          if($widget.find('.gridder-show').length > 0) {
						scrollToTop = $widget.find('.gridder-show').offset().top - stickyHeight - 12;
