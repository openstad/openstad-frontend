...
module.exports =  function (req, res, next) {
  ...
  if (globalData.siteId) {
    ...
    if (globalData.cacheIdeas) {
      let cacheKey = 'target-audience-' + globalData.siteId;
      targetAudiences = cache.get(cacheKey);
    }

    if (Array.isArray(targetAudiences)) {
      req.data.openstadTargetAudience = targetAudiences;
      next();
    } else {

      var options = {
        uri: `${apiUrl}/api/site/${globalData.siteId}/target-audience`,
        headers: headers,
      ...
      rp(options)
        .then(function (response) {
          ...
          req.data.openstadTargetAudience = response.records;

          // set the cache
          if (globalData.cacheIdeas) {
            cache.set('target-audience-' +req.data.global.siteId, response.records, {
              life: cacheLifespan
            });
          }
          next();
