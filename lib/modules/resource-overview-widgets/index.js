    ...
    const superLoad = self.load;
      ...
      const promises = [];
        ...
        widget.renderString = (data, activeResource) => {
         ...
         widget.isBefore = (date, time, unit, string) => {
            time = time ? time : 15;
            unit = unit ? unit : 'minutes';
            const dateTimeAgo = moment().subtract(time, unit);
            return moment(date, PARSE_FORMAT).isBefore(dateTimeAgo);
         };

         // expects sql date format
         widget.isAfter = (date, time, unit, string) => {
            time = time ? time : 15;
            unit = unit ? unit : 'minutes';
            const dateTimeAgo = moment().subtract(time, unit);
            return moment(date, PARSE_FORMAT).isAfter(dateTimeAgo);
         };
