  ...
  construct: function(self, options) {
    ...
    const superLoad = self.load;
      ...
      const promises = [];
        ...
        const options = {
          uri: `${apiUrl}/api/site/${req.data.global.siteId}/${resource}?${new URLSearchParams(params).toString()}`,
          headers: {
    ...
    self.formatPaginationUrls = (pageCount, baseUrl, defaultParams) => {
      ...
      for (let i = 0; i < pageCount;  i++) {
        ...
        urls.push(`${baseUrl}?${new URLSearchParams(Object.assign(defaultParams, {
          page: i
        })).toString()}`);
      }
