  ...
  addFields: [
   {
    ...
    },
    {
      name: 'mapPolygon',
      type: 'string',
      label: 'Polygon overlay voor de kaart',
      textarea: true
    }
 ],
 construct: function(self, options) {
   ...
   self.load = (req, widgets, callback) => {
     ...
     widgets.forEach((widget) => {
        widget.mapPolygon = widget.mapPolygon ? JSON.parse(widget.mapPolygon) : widget.mapPolygon
     });
   ...
   //    modBreak: req.body.modBreak,
     ...
     let postUrl = `${apiUrl}/api/site/${siteId}/idea`;
     postUrl = req.body.ideaId ? `${postUrl}/${req.body.ideaId}` : req.body.ideaId;
     ...
     rp(options)
       .then(function (response) {
          ...
          //make sure no redirect outside domain is allowed
          //redirect = redirect.replace('://', '');


          console.log('===> redirectUrl.protocol', redirectUrl.protocol);
