...
module.exports = {
  construct: function(self, options) {
    ...
    self.pageBeforeSend = (req, callback) => {
      ...
      const username = 'user-openstad-' + req.data.openstadUser.id;
      const dummyEmail = username + '@openstad.org';

      // if logged in to aposthrophecms, move on
      if (req.user && req.user.email === dummyEmail) {
        return callback();
      // logout CMS when apostropheUser is different then openstadUser
      } else if  (req.user && req.user.email === dummyEmail) {
        req.apos.logout();
      }

      self.apos.users.find(req, { username: username }).permission(false).toObject(function(err, aposUser) {
