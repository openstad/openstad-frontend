          ...
          rp(options)
            .then(function (ideas) {
              ...
              ideas = ideas.map((idea) => {
                idea.fullUrl = `${appUrl}/${ideaSlug}/${idea.id}`;
                idea.overviewUrl = `${appUrl}/${ideaOverviewSlug}?ideaId=${idea.id}`;
                return idea;
              });
    ...
    */
    ...
    self.apos.app.get('/oauth/login', (req, res, next) => {
        ...
        req.session.save(() => {
          ...
          const url = apiUrl + '/oauth/site/'+req.data.global.siteId+'/login?redirectUrl=' + fullUrl;
          res.redirect(url);
        })
