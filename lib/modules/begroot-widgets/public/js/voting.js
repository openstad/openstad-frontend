...
if (votingContainer !== null) {
  ...
  function addIdeaToSelection(id) {
    ...
    let storeSelection = votingType == 'budgeting-per-theme' ? themes.reduce( function(result, theme) { return result.concat( theme.currentSelection ) }, []) : currentSelection;
  ...
  function removeIdeaFromSelection(id) {
    ...
    let storeSelection = votingType == 'budgeting-per-theme' ? themes.map( function(theme) { return theme.currentSelection } ) : currentSelection;
	  openstadSetStorage('currentSelection', storeSelection)
	    ...
	    switch(currentStep) {
          ...
          themes.forEach( function(theme) {
            let buttons = document.querySelectorAll('.theme-button');
              ...
              if (buttons[i].querySelector('.theme-button-text').innerHTML == theme.value) {
          ...
          if ( votingType == 'budgeting-per-theme' ) {
            ...
            themes.forEach( function(theme, i) {
              ...
              // if ( i > 0 && theme.currentSelection.length > 0 ) createOverview(theme.currentSelection, theme.initialAvailableBudget, theme.availableBudgetAmount, theme.value);
            });
          } else {
  ...
  function isSelectionValid() {
    ...
    switch (votingType) {
      ...
      case 'budgeting-per-theme':
        let completeSelection = themes.reduce( function(result, theme) { return result.concat( theme.currentSelection ) }, []);
        let result = completeSelection.length > 0;
        themes.forEach( function(theme, i) {
