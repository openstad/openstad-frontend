...
function updateList() {
  ...
  var activeGebied = activeFilter ? activeFilter.value : '';
	...
	sortedElements.forEach( function(element) {
		...
		console.log('elementGebied', elementGebied);
		console.log('activeThema', activeThema);
		console.log('activeGebied', activeGebied);
...
function updateListElements() {
	...
	function updateElement(element) {
		...
		if (currentSelection.indexOf( element.ideaId ) != -1) {
			...
			console.log('elements buttons', elements);
			for (var i=0; i<elements.length; i++) {
				var el = elements[i];
...
function handleClick(event) {
	...
	console.log('ideaElement', ideaElement);
	console.log('buttonaddIdeaToSelection', buttonaddIdeaToSelection);
