...
if (westbegrootContainer) {
	...
	function updateBudgetDisplay() {
		...
		if (availableBudgetAmount == 0) {
			if (usedWidth > totalWidth ) {
				currentBudgetSelectionForWidth[currentBudgetSelectionForWidth.length-1].budgetBarWidth -= usedWidth - totalWidth ;
			}
			if (usedWidth < totalWidth ) {
	...
	function submitBudget() {
		...
		var url = '/api/site/'+siteId+'/budgeting';
	    ...
	    error: function(error) {
				addToClassName(document.querySelector('#waitLayer'), 'hidden');
				console.log('Request failed', error);
				var message = 'Het opslaan van je stem is niet gelukt: ' + ( error && error.responseJSON && && error.responseJSON.error && error.responseJSON.error.message ? error.responseJSON.error.message : error );
	...
	function updateList() {
		...
		var activeGebied = document.getElementById('filterSelector') ? document.getElementById('filterSelector').value : '';
	...
	function updateListElements() {
		...
		if (gridderShow) {
			if (gridderShow.querySelector('.this-idea-id')) {
	...
	function displayIdeaOnHash () {
		...
		var isOpen =  $('#idea-' + ideaIdId).hasClass('selectedItem');
		var scrollToTop;
		...
		if (isOpen) {
			...
			if (ideaIdId && document.querySelector('#idea-' + ideaIdId) && document.querySelector('#idea-' + ideaIdId).querySelector('.button-read-more')) {
				document.querySelector('#idea-' + ideaIdId).querySelector('.button-read-more').click();
	...
	function showInfoBewonersWest() {
		if (document.querySelector('#info-bewoners-west')) {
