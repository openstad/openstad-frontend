...
function submitNewIdea(form) {
	...
	if (!user || !user.jwt) return showError('Je bent niet ingelogd');
	var jwt = user.jwt;

	var url = apiUrl + '/site/' + '2' + '/idea';
...
function openNextStep() {
	...
	document.querySelector('#info-block .content').scrollTo(0, document.querySelector('#info-block .content').offsetHeight)
}
...
function areAllStepsOpened() {
	...
	if (allStepsAreOpened) {
		document.querySelector('#info-block').querySelector('#nextButton').style.display = 'none';
		document.querySelector('#info-block').querySelector('#submitButton').style.display = 'block';
	} else {
		document.querySelector('#info-block').querySelector('#nextButton').style.display = 'block';
		document.querySelector('#info-block').querySelector('#submitButton').style.display = 'none';
	}
}
