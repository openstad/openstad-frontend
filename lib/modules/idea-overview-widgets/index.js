  ...
  construct: function(self, options) {
    options.arrangeFields = (options.arrangeFields || []).concat([
      {
        name: 'general',
        label: 'Algemeen',
        fields: ['displayType', 'fallBackToMapImage', 'defaultImage']
      },
      {
        name: 'gridder',
        label: 'Uitklapper',
    ...
    const superLoad = self.load;
     ...
     self.output = function(widget, options) {
       ...
       widget.formatImageUrl = function (image, location, widget, width, height, crop) {
         let url;

         if (image) {
           url = image + '/:/rs=w:'+ width + ',h:' + height;
           url =  crop ? url + ';cp=w:' + width + ',h:' + height : url;
         } else if (location && widget.fallBackToMapImage) {
