...
module.exports = (self, options) => {
        ...
        doc.body.items.forEach((item) => {
            ...
            items.forEach(async (item) => {
                if(item.type === widgetName) {
                    try {
                        await self.apos.openstadApi.updateSiteConfig(req.data.global.siteConfig, item, apiSyncFields);
                    } catch(error) {
    ...
    self.load = async function(req, widgets, next) {
        ...
        widgets.map(async (widget) => {
            return self.apos.openstadApi.syncApiFields(widget, apiSyncFields, req.data.global.siteConfig, req.data.global.workflowLocale);
        });
