  ...
  construct: function(self, options) {
    ...
    self.load = function(req, widgets, next) {
        ...
        const globalData = req.data.global;
	      ...
	      widgets.forEach((widget) => {
            ...
            // Todo: refactor this to get recourceId in a different way
    				const idea = req.data.ideas ? req.data.ideas.find(idea => idea.id === parseInt(req.query.recourceId, 10)) : null;
   ...
   self.route('post', 'submit', function(req, res) {
     ...
     if (req.body.action && (req.body.action === 'UPDATE_STATUS' || req.body.action === 'MODBREAK') ) {
       ...
       rp({
           method: 'PUT',
           uri: `${postUrl}/${req.body.recourceId}`,
           headers: httpHeaders,
           body: data,
       ...
       rp({
           method: req.body.recourceId ? 'PUT' : 'POST',
           uri: req.body.recourceId ? `${postUrl}/${req.body.recourceId}` : postUrl,
           headers: httpHeaders,
           body: data,
           json: true // Automatically parses the JSON string in the response
       })
