...
const csvHeaders = {
  ...
  estimate: process.env.EXPORT_CSV_ESTIMATE || 'inschatting kosten',
  advice: process.env.EXPORT_CSV_ADVICE || 'advies',
};
...
module.exports = (self, options) => {
  ...
  self.generateCsv = (ideas) => {
    const mappedIdeas = ideas.map((idea) => {
        ...
        row[csvHeaders.advice] = idea.extraData.advice;
        row[csvHeaders.estimate] = idea.extraData.estimate;
        row[csvHeaders.phone] = idea.extraData.phone;
