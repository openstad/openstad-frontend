...
// ----------------------
	...
	.get(function(req, res, next) {
		...
		if ( typeof confirmed !== 'undefined' ) where.confirmed = ( confirmed == 'true' );
		console.log(where);
		db.NewsletterSignup
			.scope(...req.scope)
			.findAll({ where })
			.then( found => {
				return found.map( entry => {
					let json = {
						...
						signoutToken: entry.signoutToken,
					}
					return json;
				});
			})
...
// -----------------------
	...
	.post(function(req, res, next) {
		let isActive = req.site && req.site.config && req.site.config.newslettersignup && req.site.config.newslettersignup && req.site.config.newslettersignup.isActive;
		...
		if (req.user && req.user.email && req.user.email !== req.body.email) return next(createError('Dat is niet het emailadres waarmee je bent ingelogd'));
		return next();
	})
	.post(function( req, res, next ) {
		db.NewsletterSignup
				...
				if (found) {
					if (!found.externalUserId && req.user.email == found.email) {
						found
							...
							.then(result => {
								return next(createError('Je bent al aangemeld. Je aanmelding is nu ook bevestigd.'));
							})
							.catch(next)
					} else {
						return next(createError('Je bent al aangemeld'));
					}
				} else {
					return next();
				}
...
router.route('/confirm$')
	...
	.post(function(req, res, next) {
		if (!req.body.confirmToken || !req.body.confirmToken.match(/^[a-zA-Z0-9]{256}$/)) return next(createError(401, 'Token niet gevonden'))
		return next();
	})
	.post(function(req, res, next) {
		db.NewsletterSignup
			...
			.then(found => {
				if (!found) return next(createError(401, 'Aanmelding niet gevonden'))
				found
					.update({
						confirmed: true,
						confirmToken: null,
...
router.route('/signout$')
	...
	.post(function(req, res, next) {
		if (!req.body.signoutToken || !req.body.signoutToken.match(/^[a-zA-Z0-9]{256}$/)) return next(createError(401, 'Token niet gevonden'))
		return next();
	})
	.post(function(req, res, next) {
		db.NewsletterSignup
...
router.route('/:newslettersignupId(\\d+)')
	.all(function(req, res, next) {
		var newslettersignupId = parseInt(req.params.newslettersignupId) || 1;
		...
		db.NewsletterSignup
			...
			.then(found => {
				if ( !found ) throw new Error('Aanmelding niet gevonden');
